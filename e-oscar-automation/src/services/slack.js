...
if (slackToken && slackToken !== 'test-token') {
  ...
  logger.warn('SLACK_TOKEN not provided or is test token. Slack integration will be simulated.');
}
...
async function getOTPFromSlack() {
  ...
  if (process.env.DEBUG_MODE === 'true') {
    logger.info('DEBUG MODE: Returning mock OTP code');
    return '123456';
  }
...
async function sendMessage(channel, text, blocks = []) {
  // Handle debug mode
  if (process.env.DEBUG_MODE === 'true') {
    logger.info(`DEBUG MODE: Would send to Slack: "${text}"`);
    return { ok: true };
  }
